...
Плоская граница — без вложенных подрегионов, только `ru` → `ru-kda` / `ru-sta`:
```
region def ru ru-kda|ru ru-sta
region save
```
Репитер член `ru`, `ru-kda`, `ru-sta`. Scoped-трафик между ветками работает через общий родитель `ru`.
